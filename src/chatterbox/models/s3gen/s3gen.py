...
# Set to True to enable verbose memory and tensor logging (useful for debugging)
...
def clear_mps_memory():
    """Clear MPS memory with synchronization to help macOS manage memory."""
    import gc
    gc.collect()
...
def get_resampler(src_sr, dst_sr, device):
    log_message(f"[CACHE] Creating resampler {src_sr}->{dst_sr} on {device}")
...
class S3Token2Wav(S3Token2Mel):
    ...
    def flow_inference(
        ...
        result = super().forward(speech_tokens, ref_wav=ref_wav, ref_sr=ref_sr, ref_dict=ref_dict, finalize=finalize)
    ...
    def hift_inference(self, speech_feat, cache_source: torch.Tensor = None):
        ...
        result = self.mel2wav.inference(speech_feat=speech_feat, cache_source=local_cache_source)
        ...
        log_message("[HIFT_INFERENCE] Performing cleanup...")
        del local_cache_source

        # Detach outputs to avoid holding computation graph
        ...
        log_memory("end_hift_inference_after_cleanup")
    ...
    def inference(
        ...
        # Pre-inference cleanup with MPS synchronization
        ...
        if DEBUG_LOGGING:
            ...
            log_message(f"before_flow_inference: {before_flow_inference:.1f}MB")
        ...
        if DEBUG_LOGGING:
            ...
            log_message(f"after_flow_inference: {after_flow_inference:.1f}MB")
        log_tensor_info(output_mels, "inference_output_mels")

        # Mid-inference cleanup with sync to release flow matching memory
        clear_mps_memory()
        ...
        log_message("[INFERENCE] Final cleanup...")
        log_memory("before_final_cleanup")

        # Delete intermediate tensors explicitly
        del output_mels

        # Final cleanup with MPS synchronization
        clear_mps_memory()
        ...
        if DEBUG_LOGGING:
            ...
            log_message(f"[INFERENCE] Full inference complete. Final memory: {final_memory:.1f}MB")
